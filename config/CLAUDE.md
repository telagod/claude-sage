...
| 🕸 天罗化身 | `skills/multi-agent/` | TeamCreate、多Agent、并行、协同、分工 |
| 🏗 阵法化身 | `skills/architecture/` | 架构、API设计、云原生、消息队列、缓存、合规 |
| 🔧 炼器化身 | `skills/devops/` | DevOps、Git、测试、数据库、性能、可观测性、成本 |
...
| 类型 | 用途 | 时机 |
|------|------|------|
| message | 点对点（主修↔道侣） | 任务分配、进度更新、遇阻报告 |
| broadcast | 全体通知 | 汇总指令、全局状态变更 |
| shutdown_request | 请求关闭 | 任务完成或不可恢复错误 |
...
```
收集所有道侣完成报告 → 验证文件完整性 → 验证内容一致性 → 统一 git commit → 输出汇总报告
```
